...
{
	outputText("...color of '{$outputColorCode}'... ");

	foreach($outputSizes as $extension => $outputSize)
	{
		...
		outputText("...size of '{$outputSize}px' ('{$extension}')... ");
		...
		{
			//if($iKey != "bank") continue;
			$text = $iParam['code'];
			outputText("Generating icon '$iKey' ($text)... ");
			$fileName = sprintf("%s/%s/%s%s.png", $outputDir, $outputColorCode, $iKey,$extension); 
			...
			if($CONFIGS['outputBase64'])
			{
				$html = sprintf('<img src="data:image/png;base64,%s" alt="%s" />', base64_encode(file_get_contents($fileName)), $iKey);
				//echo sprintf('%s&nbsp;<textarea cols="100" rows="2">%s</textarea>', $html, htmlentities($html));				
				$htmlBuffer .= sprintf('<p>%s&nbsp;<textarea cols="100" rows="2">%s</textarea></p>', $html, htmlentities($html)). "\n\n";
			}
		}
		
		if($CONFIGS['outputBase64'])
		{
			$base64FileName = sprintf("%s/%s/%s/base64.htm", $outputDir, $outputColorCode, $outputSize); 
			file_put_contents($base64FileName, $htmlBuffer);
		}
